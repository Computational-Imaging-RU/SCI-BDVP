...
def DIP_denoiser(truth_tensor, net_input, ref_truth, Phi_tensor, y_tensor, model, optimizer, loss_fn, iter_num, y_w, tv_w,device):
    ...
    for i in range(iter_num):
        ...
        if (i+1)%25==0 and y_loss < loss_min*1.2:
        # if (i+1)%25==0:
            #loss_min = loss
            loss_min = y_loss
...
def find_optimal_lambda(x, g, y, mask, lambdas = torch.tensor([0.01, 0.05, 0.1,0.15,0.2,0.25,0.3,0.35,0.4])):
...
def GD_denoise_dip(x,y,y1,mask,mask_s,dip_iter,x_ori, it_dip_outer, y_w, tv_w,lambda1,patch_list,lr,TV_pre_denoise,TV_iter,channel,kernel,aug,weight,noise_type,decrease,device,
    ...
    logger.info('Iteration {}, PSNR gradient = {:.4f}, SSIM gradient = {:.4f}, step_size = {:.2f}'.format(it_dip_outer, psnr_torch(truth_tensor, ref_t.squeeze(0)),ssim_torch(truth_tensor, ref_t.squeeze(0)), lambda1))

    # update model weights
    # model, optimizer, loss_fn = model_load(frames)
    # print(truth_tensor.shape, net_input.shape, ref_t.shape, Phi_tensor.shape, y_tensor.shape)
    # torch.Size([8, 256, 256]) torch.Size([1, 128, 32, 32]) torch.Size([1, 8, 256, 256]) torch.Size([1, 8, 256, 256]) torch.Size([1, 256, 256])
